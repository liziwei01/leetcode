/*
 * @Author: liziwei01
 * @Date: 2023-07-11 21:24:08
 * @LastEditors: liziwei01
 * @LastEditTime: 2023-07-13 05:40:08
 * @Description: file content
 */
package sort

// quickSort sorts data[a:b] using quick sort.
func quickSort(data Interface, a, b int) {
	...
	for {
		...
		mid := simplePartition(data, a, b, pivot)
...
func simplePartition(data Interface, a, b, pivot int) (newpivot int) {
	data.Swap(a, pivot)
...
func partition(data Interface, a, b, pivot int) (newpivot int, alreadyPartitioned bool) {
	...
	// find the first element that bigger than data[pivot] on the left side
	for i <= j && data.Less(i, a) {
		i++
	}
	// find the first element that smaller than or equal to data[pivot] on the right side
	for i <= j && !data.Less(j, a) {
		j--
	}
	// if all the elements are smaller than or equal to data[pivot]
	// if at the very first round we find that all stuff are small, we will notice quick sort function about this
	if i > j {
		data.Swap(j, a)
		return j, true
	}
